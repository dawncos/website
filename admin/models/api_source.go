...
func ApiSourceGetByName(ApiSourceName string) (*ApiSource, error) {
	a := new(ApiSource)
	err := orm.NewOrm().QueryTable(GetPrefix("api_source")).Filter("source_name", ApiSourceName).One(a)
	if err != nil {
		return nil, err
	}
	return a, nil
...
func ApiSourceGetList(page, pageSize int, filters ...interface{}) ([]*ApiSource, int64) {
	...
	query := orm.NewOrm().QueryTable(GetPrefix("api_source"))
	if len(filters) > 0 {
		l := len(filters)
		for k := 0; k < l; k += 2 {
...
func ApiSourceGetById(id int) (*ApiSource, error) {
	r := new(ApiSource)
	err := orm.NewOrm().QueryTable(GetPrefix("api_source")).Filter("id", id).One(r)
	if err != nil {
		return nil, err
	}
	return r, nil
