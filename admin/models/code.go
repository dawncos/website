...
func CodeGetByName(CodeName string) (*Code, error) {
	a := new(Code)
	err := orm.NewOrm().QueryTable(GetPrefix("set_code")).Filter("code", CodeName).One(a)
	if err != nil {
		return nil, err
	}
	return a, nil
...
func CodeGetList(page, pageSize int, filters ...interface{}) ([]*Code, int64) {
	...
	list := make([]*Code, 0)
	query := orm.NewOrm().QueryTable(GetPrefix("set_code"))
	if len(filters) > 0 {
		l := len(filters)
		for k := 0; k < l; k += 2 {
...
func CodeGetByIds(ids string) ([]*Code, error) {
	list := make([]*Code, 0)
	sql := "SELECT * FROM da_set_code WHERE id in(" + ids + ")"
...
func CodeGetById(id int) (*Code, error) {
	r := new(Code)
	err := orm.NewOrm().QueryTable(GetPrefix("set_code")).Filter("id", id).One(r)
	if err != nil {
		return nil, err
	}
	return r, nil
