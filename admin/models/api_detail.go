...
func ApiTreeData(groupId int) ([]*ApiSourceList, error) {
	...
	sql := "SELECT id,source_name FROM da_api_source WHERE group_id=?"
	orm.NewOrm().Raw(sql, groupId).QueryRows(&list)
	apiList := make([]*ApiSourceList, 0)
	for _, apisource := range list {
		detailList := make([]*ApiList, 0)
		detail_sql := "SELECT id,method,api_name FROM da_api_detail WHERE source_id=? AND status=3"
...
func ApiDetailGetById(id int) (*ApiDetail, error) {
	r := new(ApiDetail)
	err := orm.NewOrm().QueryTable(GetPrefix("api_detail")).Filter("id", id).One(r)
	if err != nil {
		return nil, err
	}
	return r, nil
...
func ApiFullDetailById(id int) (detail *ApiDetails, err error) {
	sql := "SELECT da_api_detail.*,a.real_name as create_name,b.real_name as update_name,c.real_name as audit_name FROM da_api_detail LEFT JOIN da_uc_admin as a ON da_api_detail.create_id=a.id  LEFT JOIN da_uc_admin as b ON da_api_detail.update_id=b.id  LEFT JOIN da_uc_admin as c ON da_api_detail.audit_id=c.id  WHERE da_api_detail.id=?"
...
func ApiDetailsGetById(id int) ([]*ApiDetails, error) {
	...
	sql := "SELECT da_api_detail.*,a.real_name as create_name,b.real_name as update_name,c.real_name as audit_name FROM da_api_detail LEFT JOIN da_uc_admin as a ON da_api_detail.create_id=a.id  LEFT JOIN da_uc_admin as b ON da_api_detail.update_id=b.id LEFT JOIN da_uc_admin as c ON da_api_detail.audit_id=c.id WHERE da_api_detail.source_id=?"
...
func ApiDetailGetList(page, pageSize int, filters ...interface{}) ([]*ApiDetail, int64) {
	...
	query := orm.NewOrm().QueryTable(GetPrefix("api_detail"))
	if len(filters) > 0 {
		l := len(filters)
		for k := 0; k < l; k += 2 {
...
func ApiChangeStatus(ids string, status int) (num int64, err error) {
	...
	sql := "UPDATE da_api_detail set status=? where id in (" + ids + ")"
