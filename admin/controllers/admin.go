...
func (self *AdminController) AjaxSave() {
	...
	self.ajaxMsg(strconv.Itoa(resetPwd), MsgOk)
}
...
func (self *AdminController) AjaxDel() {
	...
	if Admin_id == 1 {
		self.ajaxMsg("超级管理员不允许操作", MsgErr)
	}
...
func (self *AdminController) Table() {
	...
	self.ajaxList("成功", MsgOk, count, list)
}
